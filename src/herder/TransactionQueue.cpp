...
)
{
    ...
    // stateIter = mAccountStates.end();
    // return AddResult(TransactionQueue::AddResultCode::ADD_STATUS_PENDING);

    // CLOG_INFO(Tx, "canAdd(): TEST BYPASS enabled, accepting tx {}",
    //           hexAbbrev(tx->getFullHash()));

    // // Important: set stateIter to end() to signal no existing account state.
    // stateIter = mAccountStates.end();

    // // No eviction list; we accept it unconditionally as pending.
    // return AddResult(TransactionQueue::AddResultCode::ADD_STATUS_PENDING);
    ...
    if (stateIter != mAccountStates.end())
    {
        ...
        if (transaction)
        {
            ...
            if (isDuplicateTx(currentTx, tx))
            {
                ...
                CLOG_DEBUG(Tx, "canAdd() returning: tx is a duplicate");
                return AddResult(
            ...
            if (tx->getSeqNum() < currentTx->getSeqNum())
            {
                ...
                // reject it
                CLOG_DEBUG(Tx, "canAdd() returning: tx is older than the one in the queue");
            ...
            if (tx->isSoroban())
            {
                ...
                CLOG_DEBUG(Tx, "canAdd() returning: rejecting Soroban transactions due to source account limit, check validity of its declared resources, and return an appropriate error message");
            ...
            if (tx->getEnvelope().type() != ENVELOPE_TYPE_TX_FEE_BUMP)
            {
                ...
                CLOG_DEBUG(Tx, "canAdd() returning: there's already a transaction in the queue");
            ...
            else
            {
                ...
                CLOG_DEBUG(Tx, "canAdd() returning: New fee-bump transaction is rejected");
                ...
                if (!canReplaceByFee(tx, currentTx, minFee))
                {
                    ...
                    CLOG_DEBUG(Tx, "canAdd() returning: canReplaceByFee");
...
)
{
    ...
    auto it = mAccountStates.find(tx->getSourceID());
    if (it == mAccountStates.end()) {
        auto ins = mAccountStates.emplace(tx->getSourceID(), AccountState{});
        it = ins.first;
    }

    // // Fake a queued transaction so Herder will include it
    // it->second.mTransaction = std::make_shared<QueuedTransaction>(
    //     tx, submittedFromSelf /*, other args if required */);
    ...
    CLOG_DEBUG(Tx, "starting tryAdd()0.5");
    ...
    CLOG_DEBUG(Tx, "mid tryAdd() 2");

    auto& oldTx = stateIter->second.mTransaction;
    CLOG_DEBUG(Tx, "mid tryAdd() 2.5");
