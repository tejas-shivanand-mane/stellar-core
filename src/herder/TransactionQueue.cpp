    ...
    if (stateIter != mAccountStates.end())
    {
        ...
        if (transaction)
        {
            ...
            if (isDuplicateTx(currentTx, tx))
            {
                ...
                CLOG_INFO(Tx, "canAdd() returning: tx is a duplicate");
                return AddResult(
            ...
            if (tx->getSeqNum() < currentTx->getSeqNum())
            {
                ...
                // reject it
                CLOG_INFO(Tx, "canAdd() returning: tx is older than the one in the queue");
            ...
            if (tx->isSoroban())
            {
                ...
                CLOG_INFO(Tx, "canAdd() returning: rejecting Soroban transactions due to source account limit, check validity of its declared resources, and return an appropriate error message");
            ...
            if (tx->getEnvelope().type() != ENVELOPE_TYPE_TX_FEE_BUMP)
            {
                ...
                CLOG_INFO(Tx, "canAdd() returning: there's already a transaction in the queue");
            ...
            else
            {
                ...
                CLOG_INFO(Tx, "canAdd() returning: New fee-bump transaction is rejected");
                ...
                if (!canReplaceByFee(tx, currentTx, minFee))
                {
                    ...
                    CLOG_INFO(Tx, "canAdd() returning: canReplaceByFee");
...
)
{
    ...
    CLOG_INFO(Tx, "starting tryAdd()0.5");
    ...
    CLOG_INFO(Tx, "mid tryAdd() 2");

    auto& oldTx = stateIter->second.mTransaction;
    CLOG_INFO(Tx, "mid tryAdd() 2.5");
